...
const BridgeDetailsTemplate = ({ bridgeData, chartData }) => {
    ...
    const [showCustomContent, setShowCustomContent] = useState(false); // New state to control custom content

    const handleButtonClick = () => {
        setShowTable(true);
        setShowCustomContent(false); // Ensure custom content is hidden when table is shown
    };

    const handleSequentialButtonClick = () => {
        setShowTable(false);
        setShowCustomContent(true); // Show custom content when sequential button is clicked
    };

    const handlePredictionButtonClick = () => {
        setShowTable(false);
        setShowCustomContent(false); // Ensure custom content is hidden when prediction button is clicked
    };
        ...
        <div>
            ...
            <div className='content-wrapper'>
                ...
                {showCustomContent && (
                    <div className='custom-content'>
                        <div className="chart-container">
                            ...
                            <Line
                                data={{
                                    labels: bridgeData[bridgeId]["sequential_data"][1], // Assuming the years are the x-axis labels
                                    datasets: [
                                        {
                ...
                <div className='data-display-box'>
                    <div>
                        Longitude: {bridgeData[bridgeId]["Location"][3]}
                    </div>
                    <div>
                        Latitude: {bridgeData[bridgeId]["Location"][4]}
                    </div>
                </div>
            </div>
        </div>
